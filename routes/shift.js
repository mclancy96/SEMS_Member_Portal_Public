...
router.post("/shift/:id/emailMinutes", middleware.isLoggedIn, async function(req, res){
	var today = new Date();
...
router.get("/shift/graveyard", middleware.isLoggedIn, middleware.notiCount, async function(req,res){
	var allShifts = await Shift.find().sort({"startDateMS": -1});
	res.render("./shift/graveyard",{currentUser:req.user, notiCount:res.locals.notiCount, allShifts:allShifts})
});

//============================================================================Shift CRUD===================================================================================
//New Form
router.get("/shift/new", middleware.isLoggedIn, middleware.notiCount, function(req,res){
	res.render("./shift/new", {currentUser: req.user, notiCount:res.locals.notiCount});
});

// //Index Route (The Full Schedule)
router.get("/shift", middleware.initializeSchedule, middleware.notiCount, function(req,res){
	var todayDate = new Date();
	var todayMS = Date.now();
	res.locals.formType = "new"
	res.render("./shift/schedule", {allShifts:res.locals.allShifts, currentUser: req.user, notiCount:res.locals.notiCount, events: res.locals.events, dutyCrews: res.locals.dc, tablings: res.locals.tablings, meetings: res.locals.meetings, socialGatherings: res.locals.socialGatherings, trainings: res.locals.trainings, today: todayDate, todayMS:todayMS, formType:res.locals.formType});
});

// //Create Route
router.post("/shift", middleware.isLoggedIn, middleware.setShiftVariables, function(req,res){
...
router.get("/shift/:id", middleware.isLoggedIn, middleware.notiCount, function(req,res){
	var todayMS = Date.now();	
	Shift.findById(req.params.id).exec( async function(err, foundShift){
		if (err){
			...
			res.render("shift/show", {shift: foundShift, notiCount:res.locals.notiCount, currentUser:req.user, todayMS:todayMS, allMembers:allMembers});
		}
	});
});

//Edit Route
router.get("/shift/:id/edit", middleware.isLoggedIn, middleware.notiCount, function(req,res){
	if(req.isAuthenticated()){
		Shift.findById(req.params.id, function(err, foundShift){
			res.locals.formType = "edit";
			res.render("shift/edit", {shift:foundShift, currentUser: req.user, notiCount:res.locals.notiCount, formType:res.locals.formType});
		})
	}
});

//Update Route
router.put("/shift/:id", middleware.isLoggedIn, middleware.setShiftVariables,function(req,res){
	var shiftName = res.locals.shiftName,
		shiftStart = {
			startDateRaw: req.body.startDate,
			startDateReadable: res.locals.startDateSet,
...
//Add member to crew
router.put("/shift/:id/crewAdd", middleware.isLoggedIn, function(req,res){
...
router.put("/shift/:id/crewRemove", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
...
router.put("/shift/:id/update/transportation", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
...
//Update atPD
router.put("/shift/:id/update/atPD", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
...
//Update numPts
router.put("/shift/:id/update/numPts", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
...
//Update numPts
router.put("/shift/:id/update/numPAs", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
...
//Updates onLocation
router.put("/shift/:id/update/onLocation", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
		var runFormPush = {	transportation: shift.crewRunForm.transportation,
			timeAtPD: shift.crewRunForm.timeAtPD,
...
//Updates leaveScene
router.put("/shift/:id/update/leaveScene", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
		var runFormPush = {	transportation: shift.crewRunForm.transportation,
			timeAtPD: shift.crewRunForm.timeAtPD,
			timeOnLocation: shift.crewRunForm.timeOnLocation,
...
//Updates leavePD
router.put("/shift/:id/update/leavePD", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
		var runFormPush = {	transportation: shift.crewRunForm.transportation,
			timeAtPD: shift.crewRunForm.timeAtPD,
			timeOnLocation: shift.crewRunForm.timeOnLocation,
...
//Updates narrative
router.put("/shift/:id/update/narrative", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
		var runFormPush = {	transportation: shift.crewRunForm.transportation,
			timeAtPD: shift.crewRunForm.timeAtPD,
			timeOnLocation: shift.crewRunForm.timeOnLocation,
...
//Updates narrative
router.put("/shift/:id/update/equipmentUsed", middleware.isLoggedIn, function(req,res){
	Shift.findById(req.params.id).then(async function(shift){
		var runFormPush = {	transportation: shift.crewRunForm.transportation,
			timeAtPD: shift.crewRunForm.timeAtPD,
			timeOnLocation: shift.crewRunForm.timeOnLocation,
...
//Update Attendance
router.put('/shift/:id/update/attendance', middleware.isLoggedIn, async function(req,res){
...
router.put('/shift/:id/update/attendance/add', middleware.isLoggedIn, async function(req,res){
	if(typeof req.body.attendanceEditCheckBoxes == "object"){
...
router.put('/shift/:id/update/attendance/remove', middleware.isLoggedIn, function(req,res){
...
// 	}, 
//Captain Topics
router.put("/shift/:id/update/cptTopics", middleware.isLoggedIn, function(req, res){
	Shift.findById(req.params.id).then(async function(shift){
...
//Lieutenant Topics
router.put("/shift/:id/update/ltTopics", middleware.isLoggedIn, function(req, res){
	Shift.findById(req.params.id).then(async function(shift){
		var meetingMinutePush = {	
...
router.put("/shift/:id/update/tSgtTopics", middleware.isLoggedIn, function(req, res){
	Shift.findById(req.params.id).then(async function(shift){
		var meetingMinutePush = {	
			cptTopics: shift.meetingMinutes.cptTopics,
			ltTopics: shift.meetingMinutes.ltTopics,
...
router.put("/shift/:id/update/sSgtTopics", middleware.isLoggedIn, function(req, res){
	Shift.findById(req.params.id).then(async function(shift){
		var meetingMinutePush = {	
			cptTopics: shift.meetingMinutes.cptTopics,
			ltTopics: shift.meetingMinutes.ltTopics,
...
//Advisor Topics
router.put("/shift/:id/update/advisorTopics", middleware.isLoggedIn, function(req, res){
	Shift.findById(req.params.id).then(async function(shift){
		var meetingMinutePush = {	
			cptTopics: shift.meetingMinutes.cptTopics,
			ltTopics: shift.meetingMinutes.ltTopics,
...
//Member Topics
router.put("/shift/:id/update/memberTopics", middleware.isLoggedIn, function(req, res){
	Shift.findById(req.params.id).then(async function(shift){
		var meetingMinutePush = {	
			cptTopics: shift.meetingMinutes.cptTopics,
			ltTopics: shift.meetingMinutes.ltTopics,
