...
router.get("/", middleware.isLoggedIn, middleware.notiCount, async function(req,res){
	...
	res.render("./landing", {currentUser: req.user, notiCount:res.locals.notiCount, foundShifts:foundShifts, officers:officers});
});
...
//Officer Dashboard
router.get("/officerDashboard", middleware.isOfficer, middleware.notiCount, async function(req, res){
	var officers = await User.find({accessLevel: "officer"});
	...
	res.render("./officerDashboard", {currentUser: req.user, officers:officers, allUsers:allUsers, crewChiefs:crewChiefs, emts:emts, nonEmts:nonEmts, notiCount:res.locals.notiCount});
})
...
router.get("/notifications", middleware.isLoggedIn, function(req, res){
	Notification.find({"created": {$gte: new Date((new Date().getTime() - (2592000000)))}}).sort({created: -1}).exec(function(err, allNotifications){
		res.render("notifications", {notifications: allNotifications, notiCount: res.locals.notiCount, currentUser: req.user});
