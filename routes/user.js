...
router.get('/user/eboard', middleware.isLoggedIn, middleware.notiCount, async function(req, res) {
	var officers = await User.find({accessLevel: "officer"});
	res.render('./user/eboard', {currentUser: req.user, notiCount:res.locals.notiCount, officers:officers});
});

router.get('/user/alumni', middleware.isLoggedIn, middleware.notiCount, async function(req, res) {
	var alumni = await User.find({status:"Alumni"});
	res.render('./user/alumni',{currentUser: req.user, notiCount:res.locals.notiCount, alumni:alumni});
});

router.get('/user/contact_list', middleware.isLoggedIn, middleware.notiCount,
	function(req,res){
		...
		res.render('./user/contact_list', {user:allUsers, currentUser: req.user, notiCount:res.locals.notiCount});
		}
	})
});

router.get("/user/everyMember", middleware.isLoggedIn, middleware.notiCount, function(req, res){ 
	User.find({}, function(err, allUsers){
		if(err){
			req.flash("error", err.message)
			res.redirect("back");
		} else {
			res.render("./user/everyMember", {user:allUsers, currentUser: req.user})
		}
	});
});

router.get("/user/top30", middleware.isLoggedIn, middleware.notiCount, async function(req,res){
	var topMembers = await User.find({}).sort({"hours.totalHours": -1}).limit(30);
	res.render("./user/top30", {topMembers:topMembers, currentUser: req.user, notiCount:res.locals.notiCount});
});

router.get("/user/hours", middleware.isOfficer,async function(req,res){
	var allMembers = await User.find({onRoster: true}).sort({lastName:-1});
		res.render("./user/hours", {currentUser: req.user, notiCount:res.locals.notiCount, allMembers:allMembers});
});

router.get('/user/alumni', middleware.isLoggedIn, middleware.notiCount,
	function(req, res) {
		res.render('./user/alumni', {currentUser: req.user, notiCount:res.locals.notiCount});
	}
);

//==============================================================New Member Temporary Account Routes:==========================================================

router.get("/user/add_new_member", middleware.isOfficer, middleware.notiCount, function(req, res){
	NewMember.find({}, function(err, allNewUsers){
		if(err){
			req.flash("error", err.message)
			res.redirect("back");
		} else {
			res.render("./user/add_new_member", {newUser:allNewUsers, currentUser: req.user, notiCount:res.locals.notiCount});
		}
	})
});
...
router.post("/user/add_new_member", middleware.noUser, middleware.isOfficer,	
...
router.get("/user", middleware.isLoggedIn, middleware.notiCount,//middleware.isOfficer, 
	function(req,res){
		User.find({onRoster: true}, function(err, allUsers){
		if(err){
			req.flash("error", err.message)
			...
			res.render("./user/roster", {user:allUsers, currentUser: req.user, notiCount:res.locals.notiCount}); 
		}
	})
});
...
router.get('/user/:id', middleware.isLoggedIn, middleware.notiCount, //middleware.idIsCurrentUserOrOfficer,
	function(req, res) {
	...
	if (err){
		...
		res.render("./user/show", {today: today, user: foundUser, todayRaw: todayRaw, currentUser: req.user, notiCount:res.locals.notiCount})
	}
	})
});

//Edit Route
router.get('/user/:id/edit', middleware.isLoggedIn, middleware.notiCount, //middleware.idIsCurrentUserOrOfficer,
	function(req, res) {
	...
	if (err){
		...
		res.render("./user/edit", {user:foundUser, currentUser: req.user, cprExpDateSet:cprExpDateSet, emtExpDateSet:emtExpDateSet, dlExpDateSet:dlExpDateSet, notiCount:res.locals.notiCount, formType:res.locals.formType})
	}});
}
});
		...
		if (err){
			...
			User.findByIdAndUpdate(req.params.id, updatedData, async function(err, updatedUser){
				if(err){
					...
					res.redirect("./user/", {currentUser: req.user, notiCount:res.locals.notiCount});
				} else {
...
//Destroy Route
router.delete('/user/:id', middleware.isLoggedIn, //middleware.isOfficer,
	function(req, res) {
	User.findById(req.params.id, function(err, foundUser){
		if(foundUser.status = "Delete"){
			User.findByIdAndRemove(req.params.id, function (err, userRemoved) {
				if (err) {
					req.flash("error", err.message)
					res.redirect("./user/" + req.params.id, {currentUser: req.user, notiCount:res.locals.notiCount});
				} else {
